...
## Overview
This template demonstrates how to use AI-assisted workflows (e.g., GitHub Copilot) to generate QtQuick applications with a C++17 backend. It is designed for rapid prototyping, demos, and educational purposes, following modern CMake and Qt best practices.
...
- Architecture clarification (see `copilot/architecture/`)
...
cmake --build build
# macOS example:
./build/src/app/app.app/Contents/MacOS/app
```
...
## Project Structure
- `src/app/` – Application entry point (main.cpp, QML type registration)
- `src/ui/` – QML UI components (MVVM, QML modules)
- `src/domain/` – Business logic, models (C++17)
- `tests/` – System-level tests
- `copilot/requirements/` – Requirements pipeline (see `instructions.md` in each step)
- `copilot/architecture/` – Architecture documentation, design input/output, and rationale
- `copilot/` – Guidelines, coding/testing standards, and supporting docs
...
The architecture, design decisions, and rationale are documented in [copilot/architecture/](copilot/architecture/). This folder contains:
- Stepwise architecture artifacts and instructions
- Design input/output and rationale for key decisions
- Guidance for evolving or extending the template
Refer to the `instructions.md` in that folder for details on how to use and contribute to the architecture documentation.
...
1. **Define requirements:** Start with `copilot/requirements/1_gathering/input.md`
...
**Key Pattern:** All domain logic lives in `domain.cpp/hpp` for simplicity. For larger applications, split into multiple files as needed.
...
## License
See [LICENSE](LICENSE) (MIT recommended)

## Contributing
See [CONTRIBUTING.md](CONTRIBUTING.md) for template usage and contribution rules.
